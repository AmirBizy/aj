                    ...
                    <div class="components-preview mx-auto">
                        <div class="nk-block-head nk-block-head-lg wide-sm">
                            <div class="nk-block-head-content">
                                <h3 class="nk-block-title page-title">{{ __('messages.companies') }}</h3>
                            </div>
                        </div>
                                                ...
                                                @foreach (LaravelLocalization::getSupportedLocales() as $locale_key => $locale)
                                                    <div class="tab-pane {{ $loop->first ? 'active' : '' }}" id="lang-tab-{{ $locale_key }}">
                                                        <div class="row gy-4 mt-0">
                                                            ...
                                                            <div class="col-lg-6">
                                                                <div class="form-group">
                                                                    <label class="form-label" for="translations[{{ $locale_key }}][link]">{{ __('messages.title') }}</label>
                                                                    <div class="form-control-wrap">
                                                                        <input type="text" name="translations[{{ $locale_key }}][link]" class="form-control" id="translations[{{ $locale_key }}][link]" value="{{ old("translations.$locale_key.link") }}" />
                                                                    </div>
                                                                    @error("translations.$locale_key.link")
                                                                    <small class="text-danger">{{ $message }}</small>
                                                                    @enderror
                                                                </div>
                                                            </div>
                                                            <div class="col-lg-6">
                                                                <div class="form-group">
                                                                    <label class="form-label" for="translations[{{ $locale_key }}][status]">{{ __('messages.status') }}</label>
